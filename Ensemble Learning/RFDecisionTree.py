...
# Build decision tree
def buildTree(features, OriginalFeatures, data, parent, clss, originalData, OriginalFeatures1, prevBest, depth=0, maxDepth=16, nf=2):
    depth += 1
    # Chop the tree if the maxDepth is exceeded
    if depth > maxDepth:
        maxRes = 0
        for key in OriginalFeatures1[prevBest][parent].keys():
            if OriginalFeatures1[prevBest][parent][key] > maxRes:
                maxRes = OriginalFeatures1[prevBest][parent][key]
                maxClass = key
        return maxClass
    ...
    if len(data) == 0:
        ...
        for key in OriginalFeatures1[prevBest][parent].keys():
            if key not in countClass:
                countClass[key] = OriginalFeatures1[prevBest][parent][key]
            else:
                countClass[key] += OriginalFeatures1[prevBest][parent][key]
    ...
    if maxGain==0:
        bestFeature=key
    tree = {bestFeature:{}}
    bestFeatureIndex = list(OriginalFeatures.keys()).index(bestFeature)
    del OriginalFeatures1[bestFeature]
    #del features2[bestFeature]

    for f in features[bestFeature].keys():
        #Now we fetch two new random features
        if len(list(OriginalFeatures1.keys())) > 1:
            randFeatures = RandomForest.getRandomFeatures(OriginalFeatures1, nf)
        else:
            randFeatures = OriginalFeatures1
        #newFeatures = copy.deepcopy(OriginalFeaturesEditable)
        ...
        for line in data:
            if line[bestFeatureIndex] == f:
                ...
                for key in randFeatures.keys():
                    ...
                    if attr not in randFeatures[key].keys():
                        randFeatures[key][attr] = {clss: 1}
                    else:
                        if clss not in randFeatures[key][attr].keys():
                            randFeatures[key][attr][clss] = 1
                        else:
                            randFeatures[key][attr][clss] += 1

        newOF = copy.deepcopy(OriginalFeatures1)
        # Recursive calls
        subTree = buildTree(randFeatures, OriginalFeatures, newData, f, clss, originalData, newOF, bestFeature, depth=depth, maxDepth=maxDepth)
